...
export default function RewardModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    try {
        ...
        body: JSON.stringify({ user_id: userId }), // ← ต้องรับ userId เป็น prop
        });
        ...
        {!claimed ? (
          ...
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div className="border-2 border-dashed border-[#c9a96e]/40 rounded-2xl px-8 py-4 w-full">
              <p className="text-[#c9a96e] font-bold text-xl tracking-widest font-serif">
                CLAIMED
              </p>
              {claimedTime && (
                <p className="text-white/40 text-xs mt-1">{claimedTime}</p>
              )}
            </div>
